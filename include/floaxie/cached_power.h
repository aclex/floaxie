...
namespace floaxie
{
	...
	constexpr accuracy powers_ten_exceeds[] =
	{
		less , less , less , less ,
		more , less , more , more ,
		more , more , more , less ,
	...
	inline diy_fp cached_power(int k) noexcept
	{
		...
		return diy_fp(powers_ten[index], powers_ten_e[index]);
	}

	inline accuracy cached_power_exceeds(int k) noexcept
	{
		assert(k >= -static_cast<int>(pow_0_offset));

		const std::size_t index = pow_0_offset + k;

		return powers_ten_exceeds[index];
	}
}
